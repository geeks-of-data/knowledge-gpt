def check_embedding_extractor(
    embedding_extractor, embedding_extractor_acceptable_list=None
):
    ...
    if embedding_extractor not in embedding_extractor_acceptable_list:
        raise Exception(
            f"Embedding Extractor is not allowed. "
...
def check_model_lang(model_lang, model_lang_acceptable_list=None):
    if model_lang_acceptable_list is None:
        model_lang_acceptable_list = ["en", "tr", "it"]
